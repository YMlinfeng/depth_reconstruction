...
def main(args):
    ...
    '''
    编码器 (encoder)：提取特征并将输入图像映射到潜在空间 (z)
    解码器 (decoder)：从潜在空间 (z) 生成重建图像
    判别器 (discriminator)：用于对抗性训练，区分生成图像和真实图像
    感知损失 (perceptual_loss)：用于计算 LPIPS（感知相似度）
    量化模块 (quant_conv, post_quant_conv)：用于向量量化（VQ）
    嵌入 (tok_embeddings)：用于学习离散代码表示
    交叉熵损失 (criterion)：可能用于代码预测损失
    '''
    ...
    for data_iter_step, (images, clip_image, label_cls) in enumerate(data_loader):
        ...
        # VQ-GAN 处理
        with torch.no_grad():
            _, _, _, _, _, _, xrec = model(x, clip_image, data_iter_step, step=0, is_val=True)
